...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $phonePattern = "/^09\\d{9}$/";
    $addressPattern = "/^[A-Za-z0-9\\s]+,\\s[A-Za-z\\s]+,\\s[A-Za-z\\s]+$/";
    ...
    if (!preg_match($addressPattern, $address)) {
        echo "<script>alert('Address must follow the format: Block, Subdivision, City.'); window.location.href='Register.html';</script>";
        exit;
    }
